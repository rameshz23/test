...
/* 1st Deploy*/
job("VZ_${app_farm}_1stDeploy") {
    deliveryPipelineConfiguration("VZ_${app_farm}", '1stDeploy_${host_name}')
...
steps {
  ...
  git checkout master
  sed -i "1,/\\\"version.*/s/\\\"version.*/`echo $version`/g"  '''+ """ "data_bags/app_${app_farm}/prod.json" """ + '''

  sed -i "1,/\\\"build.*/s/\\\"build.*/`echo $build`/g" '''+ """ "data_bags/app_${app_farm}/prod.json" """ + '''

  git add .
  if [ -n "$(git status --porcelain)" ]; then
  git commit -m "promoting the version from dev to qa"
  git push origin master
  knife data bag  from file ''' + """ app_${app_farm}  data_bags/app_${app_farm}/prod.json """+ '''
  echo "test"
  else
  echo 'no changes';
  fi''')
...
predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
  ...
  steps {
    ...
    downstreamParameterized {
trigger("VZ_${app_farm}_PromoteAll") {
parameters {
predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
}
}
...
credentialsId("Webexbot")
  ...
  /* FullPDV*/
job("VZ_${app_farm}_FullPDV") {
	  deliveryPipelineConfiguration("VZ_${app_farm}", 'FullPDV')
  steps {
      logRotator { numToKeep 5 }
...
/* PromoteAll job*/
job("VZ_${app_farm}_PromoteAll") {
	  deliveryPipelineConfiguration("VZ_${app_farm}", 'PromoteAll')
  steps {
      logRotator { numToKeep 5 }
  ...
  properties{
        promotions{
            promotion {
              ...
              actions {
                   downstreamParameterized  {
          trigger("VZ_${app_farm}_DeployAll") {
          parameters{
          predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
                  }}    }  }   }   }}
...
/* DeployAll job*/
job("VZ_${app_farm}_DeployAll") {
   deliveryPipelineConfiguration("VZ_${app_farm}", 'DeployAll')
  steps {
      logRotator { numToKeep 5 }
      label("${agentLabel}")
      shell
  ...
  parameters {
      ...
      wrappers {
       ...
       cleanupParameter()}
        ...
        downstreamParameterized  {
          trigger("DP_${app_type}_Promotion") {
        parameters{
          predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
                  }}
        }
...
/* Promotion job*/
job("VZ_${app_type}_Promotion") {
  deliveryPipelineConfiguration("VZ_${app_type}_Promotion", 'Promotion')
  steps {
      logRotator { numToKeep 5 }
        label("${agentLabel}")
  parameters {
  ...
  /* Runner job*/
job("VZ_${app_type}") {
 deliveryPipelineConfiguration("VZ_${app_type}_Promotion", 'Runner')
steps {
      logRotator { numToKeep 5 }
        label("${agentLabel}")
      wrappers {
...
i=0
  ...
  conditionalSteps {
    ...
    steps {
      downstreamParameterized {
                  // A comma separated list of projects to build
          trigger("VZ_${app_farm1}_1stDeploy") {
              block {
                buildStepFailure('never')
                failure('never')
                unstable('never')
