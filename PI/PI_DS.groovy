...
def app_farm="${farm}"
 ...
 /* 1st Deploy job*/
job("DS_${app_farm}_1stDeploy") {
    deliveryPipelineConfiguration("DS_${app_farm}", '1stDeploy_${host_name}')
...
steps {
  ...
  shell{
    command('''   version_no=`echo ${BUILD_NUMBER}| rev | cut -d_ -f2| rev`
  ...
  git checkout master
  sed -i "1,/\\\"version.*/s/\\\"version.*/`echo $version`/g"  '''+ """ "data_bags/app_${app_farm}/staging.json" """ + '''

  sed -i "1,/\\\"build.*/s/\\\"build.*/`echo $build`/g" '''+ """ "data_bags/app_${app_farm}/staging.json" """ + '''

  git add .
  if [ -n "$(git status --porcelain)" ]; then
  git commit -m "promoting the version from dev to qa"
  git push origin master
  knife data bag  from file ''' + """ app_${app_farm}  data_bags/app_${app_farm}/staging.json """+ '''
  echo "test"
  else
  echo 'no changes';
  fi''')
...
predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
  ...
  steps {
    downstreamParameterized {
trigger("DS_${app_farm}_PromoteAll") {
parameters {
predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
}}}  } }
     	  conditionalSteps {
...
credentialsId("Webexbot")
   ...
   /* FullPDV job*/
job("DS_${app_farm}_FullPDV") {
	  deliveryPipelineConfiguration("DS_${app_farm}", 'FullPDV')
  steps {
      logRotator { numToKeep 5 }
...
credentialsId("Webexbot")
 ...
 /* PromoteAll job*/
job("DS_${app_farm}_PromoteAll") {
	  deliveryPipelineConfiguration("DS_${app_farm}", 'PromoteAll')
  steps {
      logRotator { numToKeep 5 }
  ...
  properties{
        promotions{
            promotion {
              ...
              actions {
                   downstreamParameterized  {
          trigger("DS_${app_farm}_DeployAll") {
          parameters{
          predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
                  }}    }  }   }   }}
...
credentialsId("Webexbot")
 ...
 /* DeployAll job*/
job("DS_${app_farm}_DeployAll") {
   deliveryPipelineConfiguration("DS_${app_farm}", 'DeployAll')
  steps {
      logRotator { numToKeep 5 }
      label("${agentLabel}")
      shell
  ...
  parameters {
      ...
      wrappers {
       ...
       cleanupParameter()}
        ...
        downstreamParameterized  {
          trigger("VZ_${app_type}_Generator") {
        parameters{
          predefinedProps([BUILD_NUMBER: '$BUILD_NUMBER'])
                  }}
        }
...
/* Promotion job*/
job("DS_${app_type}_Promotion") {
  deliveryPipelineConfiguration("DS_${app_type}_Promotion", 'Promotion')
  steps {
      logRotator { numToKeep 5 }
        label("${agentLabel}")
  parameters {
           ...
           promotions{
               promotion {
                  ...
                  for (farm in farms_list){
                     ...
                     app_farm1 = app_farm1.replaceAll("\"","")
  ...
  /* Runner job*/
job("DS_${app_type}") {
 deliveryPipelineConfiguration("DS_${app_type}_Promotion", 'Runner')
steps {
      logRotator { numToKeep 5 }
        label("${agentLabel}")
      wrappers {
...
i=0
  ...
  conditionalSteps {
    ...
    steps {
      downstreamParameterized {
                  // A comma separated list of projects to build
          trigger("DS_${app_farm1}_1stDeploy") {
              block {
                buildStepFailure('never')
                failure('never')
                unstable('never')
